...
void HardFault_Handler(void)
{
    ...
    uint32_t ia = sp[24/4];
    NRF_LOG_PRINTF("Hard Fault at address: 0x%08x\r\n", (unsigned int)ia);
    while(1)
        ;
}
...
static void timers_init(void)
{
    ...
    err_code = app_timer_create(&m_sync_count_timer_id,
                                APP_TIMER_MODE_REPEATED,
                                sync_beacon_count_printout_handler);
    APP_ERROR_CHECK(err_code);
}
...
void GPIOTE_IRQHandler(void)
{
    ...
    if (NRF_GPIOTE->EVENTS_IN[1] != 0)
    {
        ...
        if (m_send_sync_pkt)
        {
            ...
            NRF_LOG("Stopping sync beacon transmission!\r\n");
        }
        else
        {
            ...
            NRF_LOG("Starting sync beacon transmission!\r\n");
        }
    }
    ...
    {        
        ...
        {
            NRF_LOG("Stopping advertising\r\n");
            sd_ble_gap_adv_stop();
            m_advertising_running = false;
        }
        else
        {
            NRF_LOG("Starting advertising\r\n");
...
static void sync_timer_button_init(void)
{
    ...
    NRF_LOG_PRINTF("Started listening for beacons.\r\n");
    NRF_LOG_PRINTF("Press Button 1 to start sending sync beacons\r\n");
}
